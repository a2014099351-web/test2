...
| （实现补充） | 原型模式 | `KWidget::clone()`，用于复制粘贴 |
...
| 补充 | 原型 | `KWidget::clone()` → 复制粘贴 |

## 6. 如何扩展一个新控件（只增文件）

对应需求「不允许修改任何现有代码，只允许新增 `.h` / `.cpp`」。  
现成样例：`imagewidget` / `sliderwidget` / `progressbarwidget`；学有余力富文本见 `richeditwidget`。
